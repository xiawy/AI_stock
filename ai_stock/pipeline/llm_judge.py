...
def _build_single_prompt(agent_role: str, news_item: dict) -> str:
    ...
    return (
        ...
        f"3. industries: 受影响的行业列表\n"
        f"4. top_stocks: 最多 3 支弹性最大的个股（code, name, elasticity 0-1）\n"
        f"5. supply_demand_signal: 供需信号 "
...
def _build_batch_prompt(agent_role: str, news_batch: list[dict]) -> str:
    ...
    return (
        ...
        f"{all_items}\n\n"
        f"---\n"
        f"请对每条新闻按顺序输出同样的字段（score, reasoning, industries, "
        f"top_stocks, supply_demand_signal），以 JSON 数组格式返回。\n"
    )
